...
public class MultiDrawRenderList {
	...
	List<VBOPool.Entry> entries = new ArrayList<VBOPool.Entry>();
	Set<VBOPool.Entry> visibleEntries = new HashSet<VBOPool.Entry>();
	...
	public MultiDrawRenderList(VBOPool vboPool) {
		...
		posBuffer = NatriumUtils.createIntBuffer(100000);
		sizeBuffer = NatriumUtils.createIntBuffer(100000);
	}
	
	public VBOPool.Entry add(CustomTessellator tessellator, boolean visible) {
		if(!tessellator.isDrawing()) {
			throw new IllegalStateException("Tessellator is not drawing!");
		}
		tessellator.setDrawing(false);
		
		if(tessellator.getVertexCount() == 0) {
			return null;
		}
		...
		if(entries.size() == 0) {
			...
			if(!this.config.equals(tessellatorConfig)) {
				throw new RuntimeException("VertexConfig Mismatch: " + this.config + " != " + tessellatorConfig);
			}
		}
